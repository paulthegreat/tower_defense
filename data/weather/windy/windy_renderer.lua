...
local WindyWeatherRenderer = class()

local GUST_INTERVAL = {4, 8}  -- Random in range in game minutes
local GUST_EFFECTS = {
   { uri = 'stonehearth:effects:wind:breeze_swirl', size = Point3(6, 5, 6) },
   { uri = 'stonehearth:effects:wind:breeze', size = Point3(5, 4, 30) },
}
local MAX_POINTS_TO_TRY_PER_SPAWN = 3
local SCREEN_MARGIN = 100
local DEFAULT_CLOUD_SPEED = 1
local WINDY_CLOUD_SPEED = 6

-- TODO: Interpolate cloud speed changes.

function WindyWeatherRenderer:initialize()
...
function WindyWeatherRenderer:destroy()
   if self._clock_trace then
      self._clock_trace:destroy()
      self._clock_trace = nil
   end
   
   _radiant.renderer.set_global_uniform('cloud_speed', DEFAULT_CLOUD_SPEED)
end

function WindyWeatherRenderer:activate()
   ...
   _radiant.renderer.set_global_uniform('cloud_speed', WINDY_CLOUD_SPEED)
end

function WindyWeatherRenderer:_update_time(now)
...
function WindyWeatherRenderer:_spawn_gust_at(location, effect)
